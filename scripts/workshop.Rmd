...
```{r}
ecoregions <- read_sf(here("/Users/dcsuh/Desktop/Ecoregions2017/Ecoregions2017.shp"))
...
```{r}
simple_map <- tm_shape(eco_temperate) + tm_polygons() + tm_shape(host_sf) + tm_dots()
simple_map
```


```{r}
ecoregions_map <- tm_shape(eco_simple) + tm_fill() + tm_shape(host_sf) + tm_dots()
ecoregions_map
```
